...
<script type="text/javascript">
	...
	function checkAllFtn(){
		if($("#checkAll").attr("checked") && $("#checkAll").attr("checked") == "checked"){
			$("input[type='checkbox']", "#listTbody").attr("checked", "checked");
		}else{
			$("input[type='checkbox']:checked", "#listTbody").removeAttr("checked");
		}
	}

	function reviewPassFtn(){
		var checkedArr = $("input[type='checkbox']:checked", "#listTbody");
		...
		for(var i = 0; i < checkedArr.length; i++){
			checkedGeneId += $(checkedArr.get(i)).val() + ",";
		}

		checkedGeneId = checkedGeneId.substring(0, checkedGeneId.length-1);
		var boardId = $("#hiddenBoardId").val();

		$.ajax({
			url:"<%=path %>/pages/analysis/sampleSourceGeneReviewPass.action",
			type:"post",
			data:{"checkedId": checkedGeneId},
			cache:false,
			dataType:"json",
			success:function(data){
				if(data.success){
					location.href="<%=path %>/pages/analysis/intoSampleSourceGeneList.action?sampleSourceGeneQuery.boardId=" + boardId;
				}
			}
		});
	...
	function reviewNotPassFtn(){
		...
		var dialogDiv = $("<div><br/><div style='float:left; display:inline;'>复核不通过原因：</div><div style='float:right; display:inline;'><textarea style='width:320px;' rows='5' id='reviewDescTxt'></textarea></div></div>");
		dialogDiv.dialog({
			autoOpen:true,
			height:240,
			width:460,
			...
			buttons:{
				"确定":function(){
					var reviewDesc = $("#reviewDescTxt", $(this)).val();
					if(reviewDesc == ""){
						alert("请输入复核不通过原因！");
					}else{
						$.ajax({
							url:"<%=path %>/pages/analysis/sampleSourceGeneReviewNotPass.action",
							type:"post",
							data:{"checkedId": checkedGeneId, "reviewDesc": reviewDesc},
							cache:false,
							dataType:"json",
							success:function(data){
								if(data.success){
      ...
      <s:form action="" method="post" id="batchReviewForm">
      	<div class="wf5" style="margin-right:20px;">
	    	<input type="checkbox" id="checkAll" onclick="checkAllFtn();"/> 全选 &nbsp;&nbsp;
	   		<input type="button" class="button_one" onclick="reviewPassFtn();" value="复核通过"/> &nbsp;&nbsp;
	   		<input type="button" class="button_one" onclick="reviewNotPassFtn();" value="复核不通过"/>
	  	</div>

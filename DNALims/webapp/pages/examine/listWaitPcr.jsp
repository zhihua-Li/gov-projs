...
function inputRecord(id){
	$("#listWaitPcrForm").attr("action", "<%=path %>/pages/examine/intoPcrRecord.action?boardInfo.id=" + id);
	$("#listWaitPcrForm").submit();
}
$(function(){
      ...
      <s:form action="listWaitPcr" method="post" id="listWaitPcrForm" name="listWaitPcrForm">
	      <table class="table_two width2 floatL marginL1">
	        ...
	        <tr>
	          <td>取样时间：&nbsp;</td>
	          <td colspan="3">
	          	<s:textfield cssClass="text_one" name="boardInfoQuery.importDateStart" id="importDateStart"/>
	          	&nbsp;至&nbsp;
	          	<s:textfield cssClass="text_one" name="boardInfoQuery.importDateEnd" id="importDateEnd"/>
	          </td>
			  <td>
      	...
      	<s:iterator value="boardInfoList" status="su">
			<tr>
				...
				<td>
					...
					<a href="javascript:void(0);" onclick="inputRecord('<s:property value="id"/>');" class="red">填写扩增记录</a>
				</td>
			</tr>
     	</s:iterator>
      </tbody>

...
$(function(){
	...
	function inputRecord(id, consignmentNo){
		$("#listWaitReviewForm").attr("action", "<%=path %>/pages/examine/intoReviewRecord.action?boardInfo.id=" + id + "&boardInfo.consignmentNo=" + consignmentNo);
      ...
      <s:form action="listWaitReview" name="listWaitReviewForm" id="listWaitReviewForm" method="post">
	      <table class="table_two width2 floatL marginL1">
	        ...
	        <tr>
	          <td>分析时间：&nbsp;</td>
	          <td colspan="3">
	          	<s:textfield cssClass="text_one" name="boardInfoQuery.analysisDateStart" id="analysisDateStart"/>
	          	&nbsp;至&nbsp;
	          	<s:textfield cssClass="text_one" name="boardInfoQuery.analysisDateEnd" id="analysisDateEnd"/>
	          </td>
			  <td>
    ...
    <fieldset class="fieldset_one">
      ...
      <thead>
        <tr>
          ...
          <th>
          	分析人&nbsp;
          	<a href="javascript:void(0);" onclick="sortByTh(this, 'analysisUserName');">
          		<s:if test="boardInfoQuery.sortColumn == 'analysisUserName'">
          			<s:if test="boardInfoQuery.sortOrder == 'ASC'">
          				<img src="<%=path %>/images/sort(-1).png"/>
          				<input type="hidden" value="DESC"/>
          			</s:if>
      ...
      <tbody>
        <s:iterator value="boardInfoList" status="su">
			<tr>
				...
				<td><s:property value="analysisUserName" /></td>
				<td>
					<input type="hidden" value="<s:property value="id"/> "/>
					<a href="javascript:void(0);" onclick="inputRecord('<s:property value="id"/>', '<s:property value="consignmentNo"/>');" class="red">填写复核记录</a>
				</td>
			</tr>
     	</s:iterator>
      </tbody>

...
function inputRecord(id){
	$("#listWaitCheckForm").attr("action", "<%=path %>/pages/examine/intoExamineCheckRecord.action?boardInfo.id=" + id);
	$("#listWaitCheckForm").submit();
}
$(function(){
      ...
      <s:form action="listWaitCheck" method="post" id="listWaitCheckForm" name="listWaitCheckForm">
	      <table class="table_two width2 floatL marginL1">
	        ...
	        <tr>
	          <td>扩增时间：&nbsp;</td>
	          <td colspan="3">
	          	<s:textfield cssClass="text_one" name="boardInfoQuery.pcrDateStart" id="pcrDateStart"/>
	          	&nbsp;至&nbsp;
	          	<s:textfield cssClass="text_one" name="boardInfoQuery.pcrDateEnd" id="pcrDateEnd"/>
	          </td>
			  <td>
    ...
    <fieldset class="fieldset_one">
      ...
      <thead>
        <tr>
          ...
          <th>
          	扩增人&nbsp;
          	<a href="javascript:void(0);" onclick="sortByTh(this, 'pcrUserName');">
          		<s:if test="boardInfoQuery.sortColumn == 'pcrUserName'">
          			<s:if test="boardInfoQuery.sortOrder == 'ASC'">
          				<img src="<%=path %>/images/sort(-1).png"/>
          				<input type="hidden" value="DESC"/>
          			</s:if>
      ...
      <tbody>
      	<s:iterator value="boardInfoList" status="su">
			<tr>
				...
				<td><s:property value="pcrUserName" /></td>
				<td>
					<input type="hidden" value="<s:property value="id"/> "/>
					<a href="javascript:void(0);" onclick="inputRecord('<s:property value="id"/>');" class="red">填写检测记录</a>
				</td>
			</tr>
     	</s:iterator>
      </tbody>
